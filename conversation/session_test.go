...
func TestSessionRequestRequiresModel(t *testing.T) {
	t.Parallel()
	s := New(&fakeStreamer{})
	_, err := s.Request(context.Background(), Request{Inputs: []Input{{Role: unified.RoleUser, Text: "hi"}}})
...
func TestSessionReplaySecondTurnReplaysFullHistory(t *testing.T) {
	...
	s := New(fs, WithModel("gpt-4o-mini"))
	stream, _ := s.Request(context.Background(), Request{Inputs: []Input{{Role: unified.RoleUser, Text: "ping"}}})
	drain(stream)
	stream, _ = s.Request(context.Background(), Request{Inputs: []Input{{Role: unified.RoleUser, Text: "again"}}})
	drain(stream)
...
func TestSessionRejectsOverlappingTurns(t *testing.T) {
	...
	stream, err := sess.Request(context.Background(), Request{Inputs: []Input{{Role: unified.RoleUser, Text: "first"}}})
	if err != nil {
		t.Fatalf("first Request() error = %v", err)
	}
	<-bs.started
	_, err = sess.Request(context.Background(), Request{Inputs: []Input{{Role: unified.RoleUser, Text: "second"}}})
...
func TestSessionCommitsToolCallsAsAssistantParts(t *testing.T) {
	...
	s := New(fs, WithModel("gpt-4o-mini"))
	stream, err := s.Request(context.Background(), Request{Inputs: []Input{{Role: unified.RoleUser, Text: "use the weather tool"}}})
	if err != nil {
		t.Fatalf("Request() error = %v", err)
	}
	drain(stream)
...
func TestSessionRecordsReasoningHistory(t *testing.T) {
	...
	s := New(fs, WithModel("gpt-4o-mini"))
	stream, err := s.Request(context.Background(), Request{Inputs: []Input{{Role: unified.RoleUser, Text: "think"}}})
	if err != nil {
		t.Fatalf("Request() error = %v", err)
	}
	drain(stream)
...
func TestSessionReasoningReplayPreservesThinkingPartsInHistoryAndRequests(t *testing.T) {
	...
	stream, err := s.Request(context.Background(), Request{Inputs: []Input{{Role: unified.RoleUser, Text: "think first"}}})
	if err != nil {
		t.Fatalf("first Request() error = %v", err)
	}
	drain(stream)
	...
	stream, err = s.Request(context.Background(), Request{Inputs: []Input{{Role: unified.RoleUser, Text: "continue"}}})
	if err != nil {
...
func TestSessionCommitsMultipleToolCallsInAssistantTurn(t *testing.T) {
	...
	s := New(fs, WithModel("gpt-4o-mini"))
	stream, err := s.Request(context.Background(), Request{Inputs: []Input{{Role: unified.RoleUser, Text: "use both tools"}}})
	if err != nil {
		t.Fatalf("Request() error = %v", err)
	}
	drain(stream)
...
func TestSessionReplayIncludesPriorAssistantToolCalls(t *testing.T) {
	...
	drain(stream)
	stream, err = s.Request(context.Background(), Request{Inputs: []Input{{Role: unified.RoleTool, ToolResult: &ToolResult{ToolCallID: "call_1", Output: `{"temp":"20C"}`}}, {Role: unified.RoleTool, ToolResult: &ToolResult{ToolCallID: "call_2", Output: `{"time":"12:00"}`}}}})
	if err != nil {
		t.Fatalf("second Request() error = %v", err)
	}
	drain(stream)
...
func TestSessionRequestNormalizesInstructionsToolResultsAndUserInputsInOrder(t *testing.T) {
	...
	s := New(fs, WithModel("gpt-4o-mini"), WithStrategy(StrategyReplay))
	stream, err := s.Request(context.Background(), Request{
...
func TestSessionPreservesExactAssistantPartOrderForMixedTurn(t *testing.T) {
	...
	s := New(fs, WithModel("gpt-4o-mini"), WithStrategy(StrategyReplay))
	stream, err := s.Request(context.Background(), Request{Inputs: []Input{{Role: unified.RoleUser, Text: "mixed turn"}}})
	if err != nil {
		t.Fatalf("Request() error = %v", err)
	}
	drain(stream)
...
func TestSessionReplayIterativeToolLoopAcrossMultipleTurns(t *testing.T) {
	...
	s := New(fs, WithModel("gpt-4o-mini"), WithStrategy(StrategyReplay))
	stream, err := s.Request(context.Background(), Request{Inputs: []Input{{Role: unified.RoleUser, Text: "start"}}})
	if err != nil {
		t.Fatalf("first Request() error = %v", err)
	}
	drain(stream)
	stream, err = s.Request(context.Background(), Request{Inputs: []Input{{Role: unified.RoleTool, ToolResult: &ToolResult{ToolCallID: "call_1", Output: `{"temp":"20C"}`}}}})
	if err != nil {
		t.Fatalf("second Request() error = %v", err)
	}
	drain(stream)
	stream, err = s.Request(context.Background(), Request{Inputs: []Input{{Role: unified.RoleTool, ToolResult: &ToolResult{ToolCallID: "call_2", Output: `{"time":"12:00"}`}}}})
	if err != nil {
...
func TestSessionFailedToolResultFollowupDoesNotCorruptPriorState(t *testing.T) {
	...
	stream, err = s.Request(context.Background(), Request{Inputs: []Input{{Role: unified.RoleTool, ToolResult: &ToolResult{ToolCallID: "call_1", Output: `{"temp":"21C"}`}}}})
	if err != nil {
		t.Fatalf("third Request() error = %v", err)
	}
	drain(stream)
...
func TestSessionReplayIterativeMultiToolLoopAcrossTurns(t *testing.T) {
	...
	drain(stream)
	stream, err = s.Request(context.Background(), Request{Inputs: []Input{
	...
	drain(stream)
	stream, err = s.Request(context.Background(), Request{Inputs: []Input{{Role: unified.RoleTool, ToolResult: &ToolResult{ToolCallID: "call_3", Output: `{"slot":"13:00"}`}}}})
	if err != nil {
		t.Fatalf("third Request() error = %v", err)
	}
	drain(stream)
...
func TestSessionNativeMixedContentFailedFollowupDoesNotAdvancePreviousResponseID(t *testing.T) {
	...
	drain(stream)
	stream, err = s.Request(context.Background(), Request{Inputs: []Input{{Role: unified.RoleTool, ToolResult: &ToolResult{ToolCallID: "call_1", Output: `{"ok":true}`}}}})
	if err != nil {
		t.Fatalf("second Request() error = %v", err)
	}
	drain(stream)
...
func TestSessionRequestUsesBuildRequestPath(t *testing.T) {
	t.Parallel()
	...
	stream, err := s.Request(context.Background(), Request{Inputs: []Input{{Role: unified.RoleUser, Text: "next"}}})
	if err != nil {
		t.Fatalf("Request() error = %v", err)
	}
	drain(stream)
